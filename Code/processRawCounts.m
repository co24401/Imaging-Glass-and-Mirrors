...
% first_count_only = If true, only accept the first arriving photon for
...
% If multiple photon detections associated with single pulse, keep only the
% first arriving photon detection.
if first_count_only
    [~, ia, ~] = unique(countData(:, [1 4]), 'row', 'stable');
    countData = countData(ia, :);
end

markerInds = find(countData(:, 1) == 1);
disp([num2str(size(markerInds, 1)) ' markers.'])
...
if generate_figure
    ...
    figure; imagesc(uu, vv, reshape(countMap, num_u, num_v)')
    set(gca, 'YDir', 'normal')
    set(gca, 'XDir', 'reverse')
    set(gca, 'ColorScale', 'log')
    colorbar;
    ...
    if savefigs
        saveas(gcf, [filedir filename '_cts.fig'])
        saveas(gcf, [filedir filename '_cts.png'])
    end
end
